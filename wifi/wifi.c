...
#ifndef WIFI_DRIVER_FW_PATH_STA_NEW
#define WIFI_DRIVER_FW_PATH_STA_NEW		NULL
#endif
#ifndef WIFI_DRIVER_FW_PATH_AP_NEW
#define WIFI_DRIVER_FW_PATH_AP_NEW		NULL
#endif
...
static const char DRIVER_MODULE_NAME_NEW[]  = WIFI_DRIVER_MODULE_NAME_NEW;
static const char DRIVER_MODULE_TAG_NEW[]   = WIFI_DRIVER_MODULE_NAME_NEW " ";
static const char DRIVER_MODULE_PATH_NEW[]  = WIFI_DRIVER_MODULE_PATH_NEW;
static const char DRIVER_MODULE_ARG_NEW[]   = WIFI_DRIVER_MODULE_ARG_NEW;
#endif
...
int is_wifi_driver_loaded() {
	...
	char *bcmtmp= "4330";
	int tmp=0;
	property_get("ro.kernel.wifi_module", bcm_module, "");
	strncpy(bcm_mod,bcm_module,4);
	ALOGE("=======QIWU=====wifi_module=%s",bcm_module);
	if (*bcm_mod != *bcmtmp)
	{
		ALOGE("=======QIWU=====bcm_mod != *bcmtmp");
		tmp=1;
	}
	...
	char line_new[sizeof(DRIVER_MODULE_TAG_NEW)+10];
#endif
	...
	if(tmp==0){
	    ...
	    while ((fgets(line_new, sizeof(line_new), proc)) != NULL) {
	        if (strncmp(line_new, DRIVER_MODULE_TAG_NEW, strlen(DRIVER_MODULE_TAG_NEW)) == 0) {
	            fclose(proc);
	            return 1;
	        }
	    }
...
if(tmp==0){
    ...
    if (insmod(DRIVER_MODULE_PATH_NEW, DRIVER_MODULE_ARG_NEW) < 0) {
        return -1;
    }
}
...
if(tmp==0){
    ...
    if (rmmod(DRIVER_MODULE_NAME_NEW) == 0) {
        int count = 20; /* wait at most 10 seconds for completion */
        while (count-- > 0) {
            if (!is_wifi_driver_loaded())
                break;
...
{	
    ...
    case WIFI_GET_FW_PATH_STA:
		if(tmp==0)
			...
			return WIFI_DRIVER_FW_PATH_STA_NEW;
    case WIFI_GET_FW_PATH_AP:
		if(tmp==0)
			return WIFI_DRIVER_FW_PATH_AP;
		else
			return WIFI_DRIVER_FW_PATH_AP_NEW;
